...
async def cmd_offri(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.effective_message.reply_text("Scegli l'asta:", reply_markup=_aste_keyboard(aste, 0))
    return SCEGLI_ASTA
...
async def _esegui_offerta(context, team, asta_id: int, importo: int, gm_id: int) -> str | None:
    ...
    # Se l'offerta è al massimo consentito, l'asta si chiude immediatamente
    if importo == settings.offerta_massima():
...
async def start_deep_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if asta["offerente_team_id"] == team["id"]:
        await update.effective_message.reply_text("❌ Sei già il miglior offerente su questa asta.")
        return ConversationHandler.END
