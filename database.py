...
def preview_annulla_ultima_offerta(asta_id: int) -> dict | None:
    """
    Simula annulla_ultima_offerta senza modificare il DB.
    Restituisce le stesse info di annulla_ultima_offerta, utile per
    controllare il cap virtuale del nuovo vincitore prima di eseguire.
    """
    with get_conn() as conn:
        offerte = conn.execute(
            "SELECT * FROM offerte WHERE asta_id=? ORDER BY id", (asta_id,)
